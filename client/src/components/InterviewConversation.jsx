...
const InterviewConversation = () => {
    ...
    const fetchInterviewStep = async (latestCandidateResponse = '') => {
      ...
      try {
        const response = await fetch('http://localhost:4000/interview-conversation', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userName,
            jobDescription,
            conversationHistory: conversationHistory + (latestCandidateResponse ? ('\n🧑: ' + latestCandidateResponse) : '')
          })
        });
    ...
    const speak = (text) => {
      if ('speechSynthesis' in window) {
        const textToSpeak = text.replace(/🤖:/g, 'Interviewer:').replace(/🧑:/g, 'Candidate:');
    ...
    return (
      <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto', color: 'white', marginTop: '40px' }}>
        <h2>Mock Big Tech Behavioral Interview</h2>
        <p>Answer the following questions. Your responses are timed.</p>
        <form onSubmit={handleSubmitJobDescription} style={{ marginBottom: '10px' }}>
          <input
            type="text"
            value={userName}
            onChange={(e) => setUserName(e.target.value)}
            placeholder="Enter your name..."
            style={{ width: '100%', marginBottom: '10px', fontSize: '1.2rem', color: 'white', backgroundColor: 'transparent', border: '1px solid #06d6a0' }}
            required
          />
          <textarea
            ...
            rows="5"
            style={{ width: '100%', fontSize: '1.2rem', backgroundColor: 'transparent', color: 'white', border: '1px solid #06d6a0', marginBottom: '10px', textAlign: 'left', textWrap: 'balance' }}
          />
        ...
        <div style={{ marginTop: '20px', border: '1px solid #ccc', color: 'white', padding: '10px', height: '300px', overflowY: 'scroll', whiteSpace: 'pre-wrap' }}>
          {conversationHistory || 'Conversation will appear here...'}
        </div>
        <div style={{ marginTop: '10px' }}>
          <button onClick={startListening} disabled={listening} style={{ marginRight: '10px' }}>
            {listening ? 'Listening...' : 'Start Listening'}
          </button>
          <button onClick={stopListening} disabled={!listening}>Stop Listening</button>
          <button className='end-interview' onClick={handleEndInterview} style={{ marginLeft: '10px' }}>End Interview</button>
          {listening && <span style={{ marginLeft: '10px' }}>Response Time: {responseTime}s</span>}
        </div>
      </div>
    );
  };
